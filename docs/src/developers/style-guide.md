...
!!! info
    The style guide is always a work in progress, and not all VASPseudopotentials code
    follows the rules. When modifying VASPseudopotentials, please fix the style violations
...
VASPseudopotentials uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/VASPseudopotentials.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the VASPseudopotentials directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to VASPseudopotentials have
