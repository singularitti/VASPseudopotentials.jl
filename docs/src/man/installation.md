...
[VASPseudopotentials](https://github.com/singularitti/VASPseudopotentials.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("VASPseudopotentials")
   ```
...
version of VASPseudopotentials, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/VASPseudopotentials.jl"
```
...
our [GitHub repository](https://github.com/singularitti/VASPseudopotentials.jl)
for new releases.
Once we release a new version, you can update VASPseudopotentials by typing

```@repl
using Pkg
Pkg.update("VASPseudopotentials")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("VASPseudopotentials")

   julia> Pkg.gc()
   ```

2. Press `Ctrl+D` to quit the current session. Start a new Julia session and
   reinstall VASPseudopotentials.
